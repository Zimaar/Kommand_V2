...
export default function OnboardingClient({
  ...
  shopParam,
  stepParam,
}: OnboardingClientProps): React.ReactElement {
  ...
  async function initiateShopify(): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setShopError(getApiErrorMessage(data, "Something went wrong. Please try again."));
        return;
      }
  ...
  async function linkWhatsApp(): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        setPhoneError(getApiErrorMessage(data, "Failed to link. Please try again."));
        return;
      }
  ...
  async function savePreferences(): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        setSaveError(getApiErrorMessage(data, "Failed to save preferences. Please try again."));
        return;
      }
